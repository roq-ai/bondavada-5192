...
import { createSavedAdvice } from 'apiSdk/saved-advices';
...
function SavedAdviceCreatePage() {
  ...
  const handleSubmit = async (values: SavedAdviceInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
      await createSavedAdvice(values);
      resetForm();
  ...
  const formik = useFormik<SavedAdviceInterface>({
    initialValues: {
      ...
      advice_id: (router.query.advice_id as string) ?? null,
    },
