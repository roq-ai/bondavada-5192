...
import { getAdviceById } from 'apiSdk/advice';
import { AdviceInterface } from 'interfaces/advice';
import { SavedAdviceListPage } from 'pages/saved-advices';

function AdviceViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<AdviceInterface>(
    () => (id ? `/advice/${id}` : null),
    () =>
      getAdviceById(id, {
        relations: ['firm', 'user'],
