...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['owner', 'team-member', 'analyst', 'administrator'];
  ...
  const { data, error, isLoading } = useSWR<FirmInterface[]>(
    ...
    () => getFirms({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    { name: 'Advice', path: '/advice', entity: 'advice', service: AccessServiceEnum.PROJECT, icon: FiMessageCircle },
    {
      name: 'Saved Advices',
      path: '/saved-advices',
      entity: 'saved_advice',
